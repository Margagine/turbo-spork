...
def train(mnist):
	...
	regularizer=tf.constrib.layers.l2_regularizer(REGULARAZTION_RATE)
	y=mnist_inference.inference(x,regularizer)
	global_step=tf.Variable(0,trainable=False)
	variable_averages=tf.train.ExponentialMovingAverage(MOVING_AVERAGE_DECAY,global_step)
	
	variable_averages_op=variable_averages.apply(tf.trainable_variables())
	cross_entropy=tf.nn.sparse_sotfmax_cross_entropy_with_logits(logits=y,labels=tf.argmax(y_,1))
	...
	with tf.Session as sess:
		...
		for i in range(TRAINING_STEPS):
			...
			if i % 1000==0:
				print('	after %d training steps,/n	loss on training batch is '%(step))
				print(loss_value)
